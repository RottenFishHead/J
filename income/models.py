...
class RecurringIncomeManager(models.Manager):
    def due_today(self):
        today = now().date()
...
class RecurringIncome(models.Model):
    ...
    objects = RecurringIncomeManager()
...
class Income(models.Model):
    ...
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    created = models.DateField(default=now)
    recurring_income = models.ForeignKey(RecurringIncome, on_delete=models.SET_NULL, blank=True, null=True, 
